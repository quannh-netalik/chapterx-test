import {
  Button,
  Col, Form, Input, Row, Upload, notification,
} from 'antd';
...
import { CustomModal, NotificationOnFinish } from './modal.styled';
...
const CreateCardModal: FC = () => {
  ...
  const openNotification = (placement: NotificationPlacement, selectedId: number) => {
    api.info({
      message: 'Created Successful!',
      description: (
        <NotificationOnFinish>
          <div>
            Design
            {' '}
            <Link to={`${config.routes.designs}?selected=${selectedId}`}>{selectedId}</Link>
            {' '}
            has been successfully created
            {' '}
          </div>
          <Link to={`${config.routes.designs}?selected=${selectedId}`}>View In Detail</Link>
        </NotificationOnFinish>
      ),
      placement,
    });
  ...
  const onFinish = useCallback((values: any) => {
    createDesign({
      ...
      openNotification('bottomRight', _result.id);
      hideCreateModal();
      cleanUp();
    });
  }, [hideCreateModal]);
